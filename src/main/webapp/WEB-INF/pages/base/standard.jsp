...
<script type="text/javascript">
	...
	//修改数据
	function doDblClickRow(rowIndex,rowData){//行号和行数据
	   $('#addStandardWindow').window({
            title: '修改收派标准',
            width: 400,
            modal: true,
	    ...
	    //$("#addStandardWindow").attr("title","修改取派标准");
	   //form回显
	...
	function commitStandardForm(){
	   //先判断表单是否校验
	  if($('#standardForm').form('validate')){//执行EasyUi校验方法
	     //通过校验
	     $('#standardForm').submit();
	  }else{
	     //没通过校验
	     $.messager.alert('警告','表单存在非法数字,请重新输入!','warning');
	  }
